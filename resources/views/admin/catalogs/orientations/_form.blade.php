<div class="space-y-6 p-6 sm:p-8">
    <div>
        ...
        <input
            id="id"
            @if (! $orientation) name="id" @endif
            type="text"
            value="{{ old('id', $orientation->IdOrientacion ?? '') }}"
            maxlength="2"
            autocomplete="off"
            @if ($orientation) readonly @else required @endif
            @class([
                'mt-2 block w-full rounded-lg border px-3 py-2.5 font-mono uppercase text-slate-950 shadow-sm outline-none transition',
                'border-slate-300 focus:border-emerald-600 focus:ring-2 focus:ring-emerald-600/20' => ! $orientation,
                'cursor-not-allowed border-slate-200 bg-slate-100 text-slate-500' => $orientation,
            ])
        >
        <p class="mt-2 text-xs leading-5 text-slate-500">
            ...
            @else
                Hasta 2 letras, números o guiones. Se guardará en mayúsculas y luego no podrá modificarse.
            @endif
        </p>
        @error('id')
    ...
    <div>
        ...
        <input
            ...
            name="descripcion"
            type="text"
            value="{{ old('descripcion', $orientation->Descrip ?? '') }}"
            required
            maxlength="15"
            autocomplete="off"
    ...
    <div>
        ...
        <label for="hab" class="flex cursor-pointer items-start gap-3 rounded-xl border border-slate-200 p-4 transition hover:border-emerald-300 hover:bg-emerald-50/40">
            <input
                ...
                type="checkbox"
                value="1"
                @checked((bool) old('hab', $orientation->Hab ?? true))
                class="mt-0.5 h-5 w-5 rounded border-slate-300 text-emerald-600 focus:ring-emerald-600"
            >
            <span>
                <span class="block text-sm font-bold text-slate-800">Orientación habilitada</span>
