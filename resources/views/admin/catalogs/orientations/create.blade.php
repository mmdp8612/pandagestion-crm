...
@section('title', 'Nueva orientación')
...
@section('content')
    <div class="max-w-3xl">
        <div>
            ...
            <h1 class="mt-2 text-3xl font-bold tracking-tight text-slate-950">Nueva orientación</h1>
            <p class="mt-2 text-sm leading-6 text-slate-600">Agregá una orientación disponible para las propiedades.</p>
        </div>

        <form method="POST" action="{{ route('admin.catalogs.orientations.store') }}" class="mt-6 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
            @csrf

            @include('admin.catalogs.orientations._form', ['orientation' => null])
